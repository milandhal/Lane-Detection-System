...
class LaneLines:
    ...
    def plot(self, out_img):
        ...
        # Handle both float and uint8 arrays
        if widget.dtype == np.float32 or widget.dtype == np.float64:
            widget = np.clip(widget * 255, 0, 255).astype(np.uint8)
            widget //= 2
            widget[0,:] = [0, 0, 255]
            widget[-1,:] = [0, 0, 255]
            widget[:,0] = [0, 0, 255]
            widget[:,-1] = [0, 0, 255]
            widget = widget.astype(np.float32) / 255.0
        else:
            widget //= 2
            widget[0,:] = [0, 0, 255]
            widget[-1,:] = [0, 0, 255]
